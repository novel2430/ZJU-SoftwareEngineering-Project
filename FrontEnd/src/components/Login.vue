...
export default {
  ...
  methods: {
    async login() {
      ...
      var url = this.$global.httpBasic+'/login-test'
        ...
        .then((res) => {
          ...
          if (data.state == 1) {
            ...
            if (this.loginData.type == 'patient') {
              this.$global.currentType = 'patient'
              this.$router.push('/chat');
            }
            // 客服登录
            else if(this.loginData.type == 'service'){
              this.$global.currentType = 'service'
              this.$global.name = this.loginData.username
